...
<head>
    ...
    #container #mid403 #content #accordion h3 {font-weight: bold;}
	  ...
	  .code_black {font-family: monospace; background-color: #333333; border: 1px solid #bdbdbd; padding: 10px; text-align: left; color: #FFF;}
	...
	$(document).ready(function() {
      ...
      (function($) {
        ...
        $('a#link-trigger').click(function() {
            $target =  $('#compreendendoosistema').parent().next();
        	if(!$target.hasClass('active')){
               //allPanels.removeClass('active').stop().slideUp();
               $target.addClass('active').stop().slideDown();
            } else {
...
<body>
	<div id="wrapper">
		<header class="header">
			...
			<div id="error-title">
				<h1>Erro 500<br><span>Erro Interno do Servidor</span></h1>
			</div>
		</header>
			...
			<div id="faq_container">
				 <dl id="accordion">
				 	<dt><a href="#">Por que estou vendo esta página?</a></dt>
						<dd>
							<p>O Erro 500 geralmente significa que o servidor encontrou uma condição inesperada que o impediu de realizar uma solicitação. Normalmente o erro ocorre quando o servidor encontra algum problema, mas não consegue ser específico sobre a condição que levou a esse erro.</p>
							<p>Em muitos casos não se trata de um problema real com o servidor, mas um problema com as informações que o servidor está sendo instruído a acessar. É possível que o erro seja causado por algum problema no seu site, o que pode exigir uma revisão adicional da nossa equipe.</p>
							<p>O erro também pode acontecer em função de algum problema no seu site, o que vai exigir uma revisão adicional da nossa equipe. Caso acredite que a situação seja essa, entre em contato com nosso suporte via <a href="https://hostgator.com.br/suporte/" title="Sistema de Ticket HostGator">ticket (e-mail)</a> informando a situação. </p>
						</dd>
					
					<dt><a id="existealgo" href="#">Existe alguma coisa que eu possa fazer?</a></dt>
						<dd>
							<p>Existem alguns motivos comuns para esse tipo de erro, incluindo problemas com a execução de algum script. Alguns são mais fáceis de detectar e corrigir do que outros.</p>

							<h4>Propriedade de Arquivos e Diretórios</h4>
							<p>O servidor espera que arquivos e diretórios sejam propriedade de um <strong>usuário cPanel</strong> específico. Caso você tenha realizado alterações na propriedade de algum arquivo via SSH, o ideal é que reajuste o proprietário e o grupo de forma adequada. </p>

							<h5>Permissões de diretórios e arquivos</h5>
							<p>O servidor geralmente espera que arquivos HTML, imagens e outras mídias tenham a permissão configurada como <strong>644</strong>. Ele espera, também, que os diretórios tenham permissões definidas como <strong>755</strong>, na maioria dos casos.</p>
							<p>(Para detalhes, confira a seção <a id="link-trigger" href="#compreendendoosistema">Compreendendo permissões de sistema de arquivos</a>)</p>
							
							<h4>Erros na sintaxe do arquivo .htaccess</h4>
							<p>É possível que você tenha adicionado regras no arquivo.htaccess que estejam em conflito umas com as outras, ou que não sejam permitidas.</p>
							<p>Caso você precise verificar alguma regra específica, você pode comentar a linha específica no .htaccess adicionando # no início da linha.</p>
							<p>Importante: lembre-se de sempre fazer um backup desse arquivo antes de fazer qualquer mudança.</p>
							<p>Por exemplo, se o .htaccess tem esse formato:</p>
							<div class="code">
								DirectoryIndex default.html<br />
							...
							<p><strong>Nota</strong>: devido ao formato de configuração dos ambientes dos nossos servidores você não pode utilizar <strong>php_value</strong> em um arquivo .htaccess.</p>

							<h4>Limites de processos simultâneos</h4>
							<p>O erro também pode ser causado pelo número alto de processos no servidor ligados à sua conta. Cada conta de um servidor compartilhado pode utilizar até 25 processos simultâneos, sejam esses processos ligados ao seu site ou a outros processos de propriedade do mesmo usuário como, por exemplo, e-mails.</p>
							<p>Com acesso SSH (shell), você pode visualizar os processos em execução na sua conta. Para fazer isso, simplesmente digite o comando</p>
							<div class="code_black">
								faux ps
							</div>
							<p>Ou digite no formato abaixo para visualizar a conta de um usuário específico (não esqueça de substituir “nome de usuário” pelo nome real):</p>
							<div class="code_black">
								ps faux | grep nome de usuário 
							</div>
							<p>Assim que tiver o ID do processo ("pid"), digite o comando abaixo para matar o processo específico (não esqueça de substituir “pid” pela identificação real do processo):</p>
							<div class="code_black">
								kill pid
							</div>
							<p>Caso a causa do Erro 500 seja o limite de processos, entre em contato com nossa equipe via <a href="https://hostgator.com.br/suporte/" title="Sistema de Ticket HostGator">ticket (e-mail)</a>. Ao enviar o ticket, certifique-se de incluir os passos necessários para que nossa equipe possa visualizar o erro em seu site. </p>
						</dd>
				
					<dt><a id="compreendendoosistema" href="#">Compreendendo o sistema de permissões de arquivos</a></dt>
						<dd class="content">						
		...
		<footer>
			...
			<a href="https://www.hostgator.com.br/hospedagem-de-sites.php?utm_source=interno&utm_medium=link&utm_campaign=page500">Conheça nossos planos</a></p>
